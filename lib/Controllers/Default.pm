...
sub index
{
    ...
    my %query;
    if ( my $tag = $cgi_obj->param('tag') ) {
        $query{query} = [ 't3.tag'   => $tag ];
    }
    ...
    my $entries                 = Models::Entries->get_entries( with_objects => ['tags'], sort_by => 'post_date DESC', limit => ENTRY_PER_PAGE, offset => $offset, %query);
    ...
    my $next_page_number       = ($page_number * ENTRY_PER_PAGE) < $total_entry_count && $current_entry_count >= ENTRY_PER_PAGE ? $page_number + 1 : '';
